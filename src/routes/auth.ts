...
authRoutes.post("/login", async (c) => {
  ...
  const rate = await checkLoginRateLimit(c.env.DB, rateKey, now);
  if (!rate.allowed) return jsonError(429, "rate_limited", "too many attempts");
...
authRoutes.put("/me/password", async (c) => {
  ...
  if (newPassword.length < 10) return jsonError(400, "weak_password", "password too short");
...
authRoutes.post("/forgot-password", async (c) => {
  ...
  if (user && user.enabled === 1) {
    ...
    await enqueueNotification(c.env, {
      ...
      payload: {
        email,
        resetUrl: `${base}/?reset_token=${encodeURIComponent(raw)}`,
      },
    });
  }
...
authRoutes.post("/reset-password", async (c) => {
  ...
  if (!token || newPassword.length < 10) {
    return jsonError(400, "invalid_request", "token and password required");
  }
...
authRoutes.post("/bootstrap-admin", async (c) => {
  ...
  if (!username || password.length < 10) {
    return jsonError(400, "invalid_request", "username and strong password required");
  }
  const vars = readVars(c.env);
  const now = nowMs();
