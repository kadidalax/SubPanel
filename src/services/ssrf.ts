...
export function assertSafeRemoteUrl(raw: string): URL {
  ...
  if (host === "localhost" || host.endsWith(".localhost") || host.endsWith(".local")) throw new Error("localhost is not allowed");
  if (host.includes(":")) {
...
export async function fetchRemoteSubscription(rawUrl: string, headers: Record<string, string> = {}, maxBytes = 5 * 1024 * 1024): Promise<{ body: string; userinfo: string | null; finalUrl: string }> {
  let current = assertSafeRemoteUrl(rawUrl).toString();
  for (let i = 0; i < 4; i++) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 15000);
    try {
      const res = await fetch(current, { method: "GET", redirect: "manual", headers: { "user-agent": headers["user-agent"] || "sub-panel/0.1", ...headers }, signal: controller.signal });
...
export function parseUserinfo(header: string | null): { upload: number | null; download: number | null; total: number | null; expire: number | null } {
  ...
  return { upload: num("upload"), download: num("download"), total: num("total"), expire: expireRaw != null ? (expireRaw < 1e12 ? expireRaw * 1000 : expireRaw) : null };
}
